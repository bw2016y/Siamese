...
import org.apache.spark.daslab.sql.execution.joins.{ShuffledHashJoinExec, SortMergeJoinExec}
import org.apache.spark.daslab.sql.internal.SQLConf

/**
 * Ensures that the [[org.apache.spark.daslab.sql.engine.plans.physical.Partitioning Partitioning]]
 ...
 * the input partition ordering requirements are met.
  *
  *
  *  确保执行计划分区与排序正确性,如果输入数据的分布或有序性无法满足当前节点的处理逻辑，则该规则
  *  会在物理计划中添加一些Shuffle操作或者排序操作来达到要求，体现在物理算子树就是加入Exchange节点/SortExec节点
 */
case class EnsureRequirements(conf: SQLConf) extends Rule[SparkPlan] {
  ...
  private def withExchangeCoordinator(
    ...
    val supportsCoordinator =
      if (children.exists(_.isInstanceOf[ShuffleExchangeExec])) {
        // 目前，ExchangeCoordinator只支持HashPartitionings
        children.forall {
        ...
        /**
          *   在这种情况下，尽管我们没有Exchange算子，但是我们可能还是需要shuffle数据（当我们有多于一个children的时候），因为
          *   这些子节点生成数据时可能不是用相同的分区函数
          */
      ...
      if (adaptiveExecutionEnabled && supportsCoordinator) {
        ...
        children.zip(requiredChildDistributions).map {
          case (e: ShuffleExchangeExec, _) =>
            // 如果是Exchange，就将coordinator作为参数传入
        ...
        // 不需要加入ExchangeCoordinator的情况，直接返回原有的children
        children
      }
  ...
  private def ensureDistributionAndOrdering(operator: SparkPlan): SparkPlan = {
    ...
    /**
      *  用于确定数据shuffle后（Post-shuffle）的分区数目
      */
    ...
    //调用TreeNode中的withNewChildren将SparkPlan中的原有子节点进行替换
