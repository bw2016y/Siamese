...
import org.apache.spark.daslab.sql.execution.aggregate.HashAggregateExec
import org.apache.spark.daslab.sql.execution.joins.{BroadcastHashJoinExec, SortMergeJoinExec}
...
case class WholeStageCodegenExec(child: SparkPlan)(val codegenStageId: Int)
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    if (rdds.length == 1) {
      val result: RDD[InternalRow] = rdds.head.mapPartitionsWithIndex { (index, iter) =>
