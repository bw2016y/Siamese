...
import org.apache.spark.daslab.sql.execution.command.ShowTablesCommand
import org.apache.spark.daslab.sql.internal.{SessionState, SharedState, SQLConf}
import org.apache.spark.daslab.sql.sources.BaseRelation
import org.apache.spark.daslab.sql.streaming.{DataStreamReader, StreamingQueryManager}
...
class SQLContext private[sql](val sparkSession: SparkSession)
   ...
   *  import org.apache.spark.daslab.sql.types._
   *  val sqlContext = new org.apache.spark.daslab.sql.SQLContext(sc)
   *
   *  val schema =
   *    StructType(
   ...
   * [[org.apache.spark.daslab.sql.streaming.StreamingQuery StreamingQueries]] active on `this` context.
   *
   * @since 2.0.0
   */
