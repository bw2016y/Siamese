...
abstract class DataType extends AbstractDataType {
  ...
  /**
    * 判断是否和other同类型，when ignoring nullability
    * (`StructField.nullable`, `ArrayType.containsNull`, and `MapType.valueContainsNull`).
    */
  private[daslab] def sameType(other: DataType): Boolean =
  ...
  /**
    ...
    * (`StructField.nullable`, `ArrayType.containsNull`, and `MapType.valueContainsNull`).
    */
  private[daslab] def asNullable: DataType

  /**
    * Returns true if any `DataType` of this DataType tree satisfies the given function `f`.
    */
  private[daslab] def existsRecursively(f: (DataType) => Boolean): Boolean = f(this)
